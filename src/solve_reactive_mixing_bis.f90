...
subroutine solve_reactive_mixing_bis(this,root,unit,mixing_ratios,mixing_waters_indices,F_mat,time_discr_tpt,&
    int_method_chem_reacts)
    use chemistry_Lagr_m, only: chemistry_c
    use aqueous_chemistry_m, only: aqueous_chemistry_c, real_array_c, int_array_c, water_mixing_iter_EE_eq_kin, &
    water_mixing_iter_EfI_eq_kin_anal_lump, mixing_iter_comp_exch, compute_c_tilde, mixing_iter_comp, int_array_c
...
!> Time loop
    ...
    !> Target waters loop
        ...
        !> We select reactive mixing subroutine depending on the nature of the chemical system and the methods to compute Jacobians and integrate in time
            if (this%chem_syst%num_kin_reacts>0) then !> equilibrium and kinetic reactions
                if (int_method_chem_reacts==1) then !> Euler explicit
                    ...
                    if (this%act_coeffs_model==0) then !> ideal
                        ...
                        !p_solver=>water_mixing_iter_EfI_eq_kin_anal_lump
                    end if
                else
            ...
            call target_waters_new(i)%solve_reactive_mixing_iter(target_waters_old_old(i)%get_c1(),mixing_ratios%cols(i-&
            this%num_ext_waters)%col_1,conc_old,F_mat(i-this%num_ext_waters),Delta_t,p_solver)
        !> Deallocate
            deallocate(conc_old)
        end do
