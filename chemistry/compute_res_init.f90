...
subroutine compute_res_init(this,indices_icon,n_icon,indices_constrains,ctot,res)
    ...
    do i=1,n_icon(4)
        ...
        res(indices_icon%cols(4)%col_1(i))=dot_product(this%chem_syst%Se(indices_constrains(ind_cstr),1:this%speciation_alg%num_prim_species),log10(this%activities(1:this%speciation_alg%num_prim_species)))+log10(ctot(indices_icon%cols(4)%col_1(i)))-log10(this%chem_syst%eq_reacts(indices_constrains(ind_cstr))%eq_cst)
    end do
end subroutine