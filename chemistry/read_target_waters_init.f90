...
subroutine read_target_waters_init(this,unit,water_types,init_sol_types,init_gas_types,react_zones,niter,CV_flag)
    use chemistry_Lagr_m
    implicit none
    class(chemistry_c) :: this
    integer(kind=4), intent(in) :: unit !> file
    type(aqueous_chemistry_c), intent(in) :: water_types(:) !> water types
    type(solid_chemistry_c), intent(in) :: init_sol_types(:) !> includes mineral and adsorption zones (in that order)
    type(gas_chemistry_c), intent(in) :: init_gas_types(:) !> initial gas boundary zones (CHEPROO)
        ...
        if (label=='end') then
            ...
            !> El do de abajo deberia estar en una subrutina
                do i=1,this%num_target_waters
                    ...
                    this%target_waters_init(tar_wat_ind)=water_types(wtype)
                    if (counter_cols==0) then
                        call this%target_waters_init(tar_wat_ind)%set_aq_phase(this%chem_syst%aq_phase)
                        call this%target_waters_init(tar_wat_ind)%set_ind_aq_phase()
                    end if
                    !> chapuza
                    if (istype>0) then
                        this%target_solids(tar_wat_ind)=init_sol_types(istype)
                        call this%target_solids(tar_wat_ind)%set_reactive_zone(react_zones(ngzns+nstype*igzn+istype))
                        call this%target_waters_init(tar_wat_ind)%set_solid_chemistry(this%target_solids(tar_wat_ind))
                        if (igzn>0) then
                            ...
                            call this%target_gases(tar_wat_ind)%set_reactive_zone(react_zones(ngzns+nstype*igzn+istype))
                            call this%target_waters_init(tar_wat_ind)%set_gas_chemistry(this%target_gases(tar_wat_ind))
                        end if
                        if (init_sol_types(istype)%reactive_zone%cat_exch_zone%num_exch_cats>0) then
                            allocate(c2_ig(this%target_waters_init(tar_wat_ind)%solid_chemistry%reactive_zone%speciation_alg%num_eq_reactions))
                            allocate(c2_init(this%target_waters_init(tar_wat_ind)%solid_chemistry%reactive_zone%speciation_alg%num_eq_reactions))
                            c2_ig=1d-16
                            call this%target_waters_init(tar_wat_ind)%compute_c2nc_from_c1_Picard(c2_ig,c2_init,niter,CV_flag)
                        ...
                        call init_gas_types(igzn)%compute_conc_gases_ideal() !> chapuza
                        this%target_gases(tar_wat_ind)=init_gas_types(igzn)
                        call this%target_gases(tar_wat_ind)%set_reactive_zone(react_zones(igzn))
                        call this%target_waters_init(tar_wat_ind)%set_gas_chemistry(this%target_gases(tar_wat_ind))
                        call solid_chem%set_reactive_zone(react_zones(igzn))
                        call this%target_waters_init(tar_wat_ind)%set_solid_chemistry(solid_chem)
                    else
                        call solid_chem%set_reactive_zone(react_zone)
                        call this%target_waters_init(tar_wat_ind)%set_solid_chemistry(solid_chem)
                    end if
                    call this%target_waters_init(tar_wat_ind)%solid_chemistry%reactive_zone%set_speciation_alg_dimensions(flag_comp)
                    call this%target_waters_init(tar_wat_ind)%solid_chemistry%reactive_zone%compute_speciation_alg_arrays(flag_aq_phase,cols)
                    ...
                    if (aux_cols(1)>0 .AND. aux_cols(2)>0) then
                        ...
                        call this%target_waters_init(tar_wat_ind)%set_ind_aq_phase()
                        this%target_waters_init(tar_wat_ind)%indices_aq_phase(aux_cols(2))=aux_cols(1)
                        this%target_waters_init(tar_wat_ind)%indices_aq_phase(aux_cols(1))=aux_cols(2)
                        this%target_waters_init(tar_wat_ind)%concentrations(aux_cols(1))=water_types(wtype)%concentrations(aux_cols(2))
                        this%target_waters_init(tar_wat_ind)%concentrations(aux_cols(2))=water_types(wtype)%concentrations(aux_cols(1))
                        this%target_waters_init(tar_wat_ind)%activities(aux_cols(1))=water_types(wtype)%activities(aux_cols(2))
                        this%target_waters_init(tar_wat_ind)%activities(aux_cols(2))=water_types(wtype)%activities(aux_cols(1))
                        this%target_waters_init(tar_wat_ind)%log_act_coeffs(aux_cols(1))=water_types(wtype)%log_act_coeffs(aux_cols(2))
                        this%target_waters_init(tar_wat_ind)%log_act_coeffs(aux_cols(2))=water_types(wtype)%log_act_coeffs(aux_cols(1))
                    end if
                    call this%target_waters_init(tar_wat_ind)%solid_chemistry%reactive_zone%compute_U_SkT_prod()
