...
!> Pre-process
    ...
    n_mins_eq=this%solid_chemistry%reactive_zone%chem_syst%num_minerals_eq
...
!> Process
    do
        ...
        if (inf_norm_vec_real((c2_new(1:n_sec_aq)-c2_old(1:n_sec_aq))/c2_old(1:n_sec_aq))<this%solid_chemistry%reactive_zone%CV_params%rel_tol) then
            CV_flag=.true.
            exit !> CV reached
