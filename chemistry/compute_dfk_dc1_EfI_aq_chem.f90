...
!> Pre-process
    allocate(dc2nc_dc1(this%speciation_alg%num_eq_reactions,this%speciation_alg%num_prim_species))
    allocate(d_log_gamma_d_I(this%speciation_alg%num_var_act_species))
!> Process
    !> First we compute d_log_gamma_d_I
        call this%compute_d_log_gamma_d_I_aq_chem(d_log_gamma_d_I)
    !> Outer product
        out_prod=outer_prod_vec(d_log_gamma_d_I,this%chem_syst%z2(1:this%speciation_alg%num_var_act_species))
    ...
    !> We separate primary and secondary variable activity species in Jacobian of kinetic reaction rates       
        drk_dc1=drk_dc(:,1:this%speciation_alg%num_prim_species)
        drk_dc2nc=drk_dc(:,this%speciation_alg%num_prim_species+1:this%speciation_alg%num_var_act_species)
    !> We compute Jacobian Newton residual
        dfk_dc1=this%speciation_alg%comp_mat(:,1:this%speciation_alg%num_prim_species)+matmul(this%speciation_alg%comp_mat(:,this%speciation_alg%num_prim_species+1:this%speciation_alg%num_var_act_species),dc2nc_dc1)-(Delta_t/porosity)*matmul(this%U_SkT_prod,drk_dc1+matmul(drk_dc2nc,dc2nc_dc1))
!> Post-process
