...
subroutine write_chemistry(this,unit)
    ...
    integer(kind=4), allocatable :: tar_sol_indices(:),dom_indices(:),ext_indices(:)
    ...
    do i=1,this%chem_syst%aq_phase%num_species
        ...
        write(unit,"(10x,*(ES15.5))") (this%target_waters_init(j)%concentrations(this%target_waters_init(j)%indices_aq_phase(i)), j=1,this%num_target_waters)
    end do
    write(unit,"(/,2x,'Initial activity coefficients aqueous species domain:'/)")
    do i=1,this%chem_syst%aq_phase%num_species
        write(unit,"(10x,*(ES15.5))") (10**this%target_waters_init(j)%log_act_coeffs(this%target_waters_init(j)%indices_aq_phase(i)), j=1,this%num_target_waters)
    end do
    ...
    if (this%chem_syst%aq_phase%ind_wat>0) then
        ...
        write(unit,"(10x,*(ES15.5))") (this%target_waters_init(j)%activities(this%chem_syst%aq_phase%ind_wat), j=1,this%num_target_waters)
        write(unit,"(/,2x,'Initial salinity domain:'/)")
        write(unit,"(10x,*(ES15.5))") (this%target_waters_init(j)%salinity, j=1,this%num_target_waters)
    end if
    if (this%chem_syst%aq_phase%ind_proton>0) then
        write(unit,"(/,2x,'Initial pH domain:'/)")
        write(unit,"(10x,*(ES15.5))") (this%target_waters_init(j)%pH, j=1,this%num_target_waters)
    end if
    !write(unit,"(/,2x,'Initial alkalinity:'/)")
    !write(unit,"(10x,*(ES15.5))") (this%target_waters_init(j)%alkalinity, j=1,this%num_target_waters)
    ...
    if (this%num_reactive_zones>0) then
        do l=1,this%num_reactive_zones
            ...
            !call this%link_target_waters_init_reactive_zone(l,dom_indices)
            ...
            write(unit,"(10x,'Se:',/)")
            !print *, this%target_waters_init(dom_indices(1))%speciation_alg%num_species
            do i=1,this%reactive_zones(l)%speciation_alg%num_eq_reactions
                write(unit,"(10x,*(F15.5))") (this%reactive_zones(l)%stoich_mat(i,j), j=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_species)
            end do
            write(unit,"(10x,'Sk:',/)")
            do i=1,this%reactive_zones(l)%chem_syst%num_kin_reacts
                write(unit,"(10x,*(F15.5))") (this%reactive_zones(l)%chem_syst%Sk(i,j), j=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%chem_syst%num_species)
            end do
            write(unit,"(10x,'U*Sk^T:',/)")
            do i=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species
                write(unit,"(10x,*(F15.5))") (this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%U_SkT_prod(i,j), j=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%chem_syst%num_kin_reacts)
            end do
            ...
            !print *, this%target_waters(dom_indices(1))%aq_phase%num_species
            do i=1,this%chem_syst%aq_phase%num_species
                write(unit,"(20x,A15)") this%target_waters(dom_indices(1))%solid_chemistry%reactive_zone%chem_syst%aq_phase%aq_species(this%target_waters(dom_indices(1))%indices_aq_phase(i))%name
            end do
            write(unit,"(/,10x,'Primary species:',/)")
            !print *, this%target_waters(dom_indices(1))%aq_phase%num_species
            do i=1,this%target_waters(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species
                write(unit,"(20x,A15)") this%target_waters(dom_indices(1))%aq_phase%aq_species(this%target_waters(dom_indices(1))%indices_aq_phase(i))%name
            end do
            !if (associated(this%target_waters_init(dom_indices(1))%solid_chemistry)) then
            !    if (this%target_waters_init(dom_indices(1))%cat_exch_zone%num_surf_compl>0) then
            !        write(unit,"(20x,A15)") this%target_waters_init(dom_indices(1))%cat_exch_zone%surf_compl(1)%name
            !    !else
            !    !    write(unit,"(20x,A15/)") this%target_waters_init(dom_indices(1))%aq_phase%aq_species(this%target_waters_init(dom_indices(1))%prim_species_indices(this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species))%name
            !    end if
            !end if
            write(unit,"(10x,'Component matrix:'/)")
            do i=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species
                write(unit,"(10x,*(F15.5))") (this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%comp_mat(i,j), j=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species)
            end do           
            write(unit,"(/,10x,'Initial concentration of components:'/)")
            !allocate(conc_comp(this%target_waters_init(dom_indices(1))%speciation_alg%num_aq_prim_species,size(dom_indices)))
            !do j=1,size(dom_indices)
                do i=1,this%target_waters_init(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species
                    !conc_comp(i,j)=dot_product(this%target_waters_init(dom_indices(j))%solid_chemistry%reactive_zone%speciation_alg%comp_mat(i,:),this%target_waters_init(dom_indices(j))%concentrations(1:this%target_waters_init(dom_indices(j))%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species))
                    write(unit,"(10x,*(ES15.5))") (dot_product(this%target_waters_init(dom_indices(j))%solid_chemistry%reactive_zone%speciation_alg%comp_mat(i,:),this%target_waters_init(dom_indices(j))%get_conc_nc()), j=1,size(dom_indices))
                end do
            !end do
            !write(unit,"(/,10x,'Initial concentration of solid components:'/)")
            !do i=1,this%target_waters_init(dom_indices(1))%speciation_alg%num_prim_species-this%target_waters_init(dom_indices(1))%speciation_alg%num_aq_prim_species
            !    write(unit,"(10x,*(ES15.5))") (this%target_waters_init(dom_indices(j))%solid_chemistry%conc_comp(i), j=1,size(dom_indices))
            !end do
            write(unit,"(/,10x,'Final concentration of components:'/)")
            !do j=1,size(dom_indices)
                do i=1,this%target_waters(dom_indices(1))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species
                    !conc_comp(i,j)=dot_product(this%target_waters_init(dom_indices(j))%speciation_alg%comp_mat(i,:),this%target_waters_init(dom_indices(j))%concentrations(1:this%target_waters_init(dom_indices(j))%speciation_alg%num_var_act_species))
                    write(unit,"(10x,*(ES15.5))") (dot_product(this%target_waters(dom_indices(j))%solid_chemistry%reactive_zone%speciation_alg%comp_mat(i,:),this%target_waters(dom_indices(j))%get_conc_nc()), j=1,size(dom_indices))
                end do
            ...
            !do i=1,size(dom_indices)
            !    write(unit,"(10x,*(ES15.5))") (dot_product(this%target_waters(dom_indices(i))%speciation_alg%comp_mat(j,:),this%target_waters(dom_indices(i))%get_conc_nc()), j=1,this%target_waters(dom_indices(1))%speciation_alg%num_prim_species)
            !end do
            !end do
            ...
            do i=1,this%reactive_zones(l)%num_minerals
                write(unit,"(10x,*(ES15.5))") (this%target_solids(dom_indices(j))%vol_fracts(i), j=1,size(dom_indices))
            end do
            ...
            do i=1,this%reactive_zones(l)%num_minerals
                write(unit,"(10x,*(ES15.5))") (this%target_solids(dom_indices(j))%concentrations(i), j=1,size(dom_indices))
            end do
            ...
            !    write(unit,"(10x,*(ES15.5))") (this%target_waters_init(dom_indices(j))%solid_chemistry%concentrations(this%reactive_zones(l)%num_minerals+i), j=1,size(dom_indices))
            !end do
            ...
            do i=1,this%reactive_zones(l)%gas_phase%num_species
                write(unit,"(10x,*(ES15.5))") (this%target_gases(dom_indices(j))%concentrations(i), j=1,size(dom_indices))
            end do
            ...
            do i=1,this%reactive_zones(l)%gas_phase%num_species
                write(unit,"(10x,*(ES15.5))") (this%target_gases(dom_indices(j))%activities(i), j=1,size(dom_indices))
            end do
            ...
            if (this%reactive_zones(l)%gas_phase%num_species>0) then
                write(unit,"(10x,*(ES15.5))") (this%target_gases(dom_indices(j))%volume, j=1,size(dom_indices))
            end if
        ...
        allocate(conc_comp(this%target_waters_init(1)%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species,this%num_target_waters))
        ...
        !do j=1,this%num_target_waters
            do i=1,this%target_waters_init(1)%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species
                write(unit,"(10x,*(ES15.5))") (dot_product(this%target_waters_init(j)%solid_chemistry%reactive_zone%speciation_alg%comp_mat(i,:),this%target_waters_init(j)%concentrations(1:this%target_waters_init(j)%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species)), j=1,this%num_target_waters) !(this%target_waters_init(dom_indices(j))%concentrations(i), j=1,size(dom_indices))
            end do
        !end do
