        ...
        !> Autentica chapuza
            ...
            if (associated(this%solid_chemistry)) then
                ...
                if (associated(this%gas_chemistry)) then
                    this%gas_chemistry%r_eq(1:this%solid_chemistry%reactive_zone%gas_phase%num_cst_act_species)=R_eq(this%solid_chemistry%reactive_zone%num_minerals_cst_act+1:this%speciation_alg%num_cst_act_species-this%chem_syst%aq_phase%wat_flag)*porosity/Delta_t !> r_eq_j=R_eq*phi_j/Delta_t
                    this%gas_chemistry%r_eq(this%solid_chemistry%reactive_zone%gas_phase%num_cst_act_species+1:this%solid_chemistry%reactive_zone%gas_phase%num_species)=R_eq(this%speciation_alg%num_cst_act_species-this%chem_syst%aq_phase%wat_flag+this%chem_syst%aq_phase%num_aq_complexes+this%chem_syst%num_redox_eq_reacts+this%solid_chemistry%reactive_zone%cat_exch_zone%num_exch_cats+1:this%solid_chemistry%reactive_zone%num_eq_reactions)*porosity/Delta_t !> r_eq_j=R_eq*phi_j/Delta_t
                end if
                    ...
                    b=matmul(this%gas_chemistry%reactive_zone%stoich_mat(:,this%speciation_alg%num_prim_species+1:this%speciation_alg%num_var_act_species),c2nc-c2nc_tilde) - matmul(this%gas_chemistry%reactive_zone%stoich_mat(:,this%speciation_alg%num_prim_species+1:this%speciation_alg%num_var_act_species),matmul(transpose(this%chem_syst%Sk(:,this%speciation_alg%num_prim_species+1:this%speciation_alg%num_var_act_species)),Delta_t*this%rk/porosity))
                    if (inf_norm_vec_real(b)<this%CV_params%zero) then
                        R_eq=0d0
                    else
                        call Gauss_Jordan(A,b,this%CV_params%zero,R_eq,err) !> linear system solver
                    ...
                    if (this%gas_chemistry%reactive_zone%gas_phase%num_gases_eq>0) then
                        this%gas_chemistry%r_eq(1:this%gas_chemistry%reactive_zone%gas_phase%num_cst_act_species)=R_eq(this%gas_chemistry%reactive_zone%num_minerals_cst_act+1:this%speciation_alg%num_cst_act_species-this%chem_syst%aq_phase%wat_flag)*porosity/Delta_t !> r_eq_j=R_eq*phi_j/Delta_t
                        this%gas_chemistry%r_eq(this%gas_chemistry%reactive_zone%gas_phase%num_cst_act_species+1:this%gas_chemistry%reactive_zone%gas_phase%num_species)=R_eq(this%speciation_alg%num_cst_act_species-this%chem_syst%aq_phase%wat_flag+this%chem_syst%aq_phase%num_aq_complexes+this%chem_syst%num_redox_eq_reacts+this%gas_chemistry%reactive_zone%num_minerals_var_act+this%gas_chemistry%reactive_zone%cat_exch_zone%num_exch_cats+1:this%gas_chemistry%reactive_zone%num_eq_reactions)*porosity/Delta_t !> r_eq_j=R_eq*phi_j/Delta_t
                    end if
            else
