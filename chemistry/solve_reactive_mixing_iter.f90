...
!> Arguments
    ...
    external :: solver !> reactive mixing iteration solver
!> Variables
        ...
        !> Time loop
            ...
            !> Target waters loop
                !do i=this%num_ext_waters+1,this%num_target_waters
                    if (abs(mixing_ratios(1)-1d0)<this%solid_chemistry%reactive_zone%CV_params%abs_tol .and. inf_norm_vec_real(mixing_ratios(2:size(mixing_ratios)))<this%solid_chemistry%reactive_zone%CV_params%abs_tol) then
                        continue
                    else
                    ...
                    !> We solve reactive mixing iteration
                        call solver(this,c1_old,this%get_c2nc(),c_tilde(this%indices_aq_phase(1:this%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species)),conc_nc,porosity,Delta_t)
                    !> We compute equilibrium reaction rates from mass balance equation
                        call this%compute_r_eq(c_tilde(this%indices_aq_phase(this%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:this%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species)),Delta_t,porosity)
                    !> Chapuza
                        ...
                        !> We compute concentrations of minerals
                            call this%solid_chemistry%compute_conc_minerals_iter(Delta_t)
                        end if
