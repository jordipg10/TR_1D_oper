...
!> Pre-process
    ...
    !allocate(conc_nc(1:this%speciation_alg%num_var_act_species))
!> Process  
    !> We compute component concentrations after mixing
        conc_comp=MATMUL(THIS%speciation_alg%comp_mat,c_tilde)
        if (inf_norm_vec_real(c_tilde(1:this%speciation_alg%num_prim_species)-c1)<this%CV_params%zero) then !> chapuza
            ...
            !> Analytical solution gypsum+calcite ideal    
                !if (abs(conc_comp(1)-this%solid_chemistry%reactive_zone%eq_reactions(3)%eq_cst/(this%solid_chemistry%reactive_zone%eq_reactions(2)%eq_cst*conc_nc(5))+(this%solid_chemistry%reactive_zone%eq_reactions(2)%eq_cst*conc_nc(5))/(this%solid_chemistry%reactive_zone%eq_reactions(1)%eq_cst*this%solid_chemistry%reactive_zone%eq_reactions(3)%eq_cst)+(conc_comp(2)*conc_nc(5))/(this%solid_chemistry%reactive_zone%eq_reactions(3)%eq_cst+conc_nc(5))+conc_nc(5))>this%CV_params%abs_tol) then
                !    error stop
                !end if
