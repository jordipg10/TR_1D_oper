subroutine link_target_waters_reactive_zone(this,i,dom_indices,ext_indices)
    ...
    flag=.true.
    do
        if (this%target_waters(this%dom_tar_wat_indices(j))%solid_chemistry%reactive_zone%num_non_flowing_species>0 .and. this%target_waters(this%dom_tar_wat_indices(j))%solid_chemistry%reactive_zone%num_non_flowing_species==this%reactive_zones(i)%num_non_flowing_species) then
            call this%target_waters(this%dom_tar_wat_indices(j))%solid_chemistry%reactive_zone%is_nf_species_in_react_zone(this%reactive_zones(i)%non_flowing_species(k),flag,nf_sp_ind)
            if (flag==.false.) then
    ...
    if (this%num_ext_waters>0) then
        ...
        flag=.true.
        do
            if (this%target_waters(this%ext_waters_indices(j))%solid_chemistry%reactive_zone%num_non_flowing_species>0 .and. this%target_waters(this%ext_waters_indices(j))%solid_chemistry%reactive_zone%num_non_flowing_species==this%reactive_zones(i)%num_non_flowing_species) then
                call this%target_waters(this%ext_waters_indices(j))%solid_chemistry%reactive_zone%is_nf_species_in_react_zone(this%reactive_zones(i)%non_flowing_species(k),flag,nf_sp_ind)
                if (flag==.false.) then
            ...
            else if (this%target_waters(this%ext_waters_indices(j))%solid_chemistry%reactive_zone%num_non_flowing_species==0 .and. this%target_waters(this%ext_waters_indices(j))%solid_chemistry%reactive_zone%num_non_flowing_species==this%reactive_zones(i)%num_non_flowing_species) then
                if (j<this%num_ext_waters) then
                    call append_int_1D_array(ext_indices,this%ext_waters_indices(j))
                    j=j+1
                else
