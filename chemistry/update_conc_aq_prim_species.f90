...
subroutine update_conc_aq_prim_species(this,Delta_c1)
    ...
    if (n_p_aq/=size(Delta_c1) .and. this%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species/=size(Delta_c1)) error stop "Dimension error in update_conc_aq_prim_species"
    
    c1_old=this%concentrations(1:n_p_aq)
    do i=1,n_p_aq
        if (this%concentrations(i)+Delta_c1(i)<=this%solid_chemistry%reactive_zone%CV_params%control_factor*this%concentrations(i)) then
            this%concentrations(i)=this%solid_chemistry%reactive_zone%CV_params%control_factor*this%concentrations(i)
        else if (this%concentrations(i)+Delta_c1(i)>=this%concentrations(i)/this%solid_chemistry%reactive_zone%CV_params%control_factor) then
