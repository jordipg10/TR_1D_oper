...
subroutine solve_reactive_mixing(this,root,unit,mixing_ratios,mixing_waters_indices,F_mat,time_discr_tpt,int_method_chem_reacts)
...
!> We select reactive mixing subroutine depending on the nature of the chemical system and the methods to compute Jacobians and integrate in time
                ...
                do i=this%num_ext_waters+1,this%num_target_waters
                    if (abs(mixing_ratios%cols(i-this%num_ext_waters)%col_1(1)-1d0)<this%target_waters(i)%CV_params%abs_tol .and. inf_norm_vec_real(mixing_ratios%cols(i-this%num_ext_waters)%col_1(2:mixing_ratios%cols(i-this%num_ext_waters)%dim))<this%target_waters(i)%CV_params%abs_tol) then
                        ...
                        if (this%chem_syst%num_kin_reacts>0) then !> equilibrium and kinetic reactions
                            ...
                            if (this%act_coeffs_model==0) then !> ideal
                                p_solver=>transport_iter_comp_EE_aq_chem_ideal !> only equilibrium reactions
                            else
                    ...
                    !> We compute equilibrium reaction rates from mass balance equation
                        call target_waters_new(i)%compute_r_eq_aq_chem(c_tilde(target_waters_new(i)%indices_aq_phase(target_waters_new(i)%speciation_alg%num_prim_species+1:target_waters_new(i)%speciation_alg%num_var_act_species)),Delta_t,F_mat(i-this%num_ext_waters))
                    !> Chapuza
    ...
    !> Time loop
        ...
        !> Target waters loop
            ...
            !> We compute equilibrium reaction rates from mass balance equation
                call target_waters_new(i)%compute_r_eq_aq_chem(c_tilde(target_waters_new(i)%speciation_alg%num_prim_species+1:target_waters_new(i)%speciation_alg%num_var_act_species),Delta_t,F_mat(i-this%num_ext_waters))
            !> Chapuza
                if (associated(target_waters_new(i)%solid_chemistry)) then
                !> We compute mass volumetric fractions of minerals from mass balance equation
                    call target_waters_new(i)%solid_chemistry%compute_mass_bal_mins(Delta_t)
