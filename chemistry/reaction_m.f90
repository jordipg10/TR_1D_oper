...
module reaction_m
    ...
    contains
       ...
       subroutine set_stoichiometry(this,stoichiometry)
            ...
            if (allocated(this%species) .and. size(stoichiometry)/=this%num_species) error stop "Dimension error in 'set_stoichiometry'"
       ...
       subroutine are_species_in_react(this,species,flag,species_ind) !> checks if all species participate in reaction
        ...
        subroutine compute_logK_dep_T(this,temp) !> based on PHREEQC
            ...
            logK=this%coeffs_logK_T(1)+this%coeffs_logK_T(2)*temp+this%coeffs_logK_T(3)/temp+this%coeffs_logK_T(4)*log10(temp)+this%coeffs_logK_T(5)/(temp**2)+this%coeffs_logK_T(6)*temp**2
            this%eq_cst=10**logK
        end subroutine
