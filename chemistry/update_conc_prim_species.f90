...
subroutine update_conc_prim_species(this,c1,Delta_c1)
    ...
    do i=1,n_p
        if (c1_old(i)+Delta_c1(i)<=this%CV_params%control_factor*c1_old(i)) then
            c1(i)=this%CV_params%control_factor*c1_old(i)
        else if (c1_old(i)+Delta_c1(i)>=c1_old(i)/this%CV_params%control_factor) then
            c1(i)=c1_old(i)/this%CV_params%control_factor
        else
