...
!> Procedure pointers
    ...
    procedure(get_c1), pointer :: p_prim=>get_c1
...
!> We select reactive mixing subroutine depending on the nature of the chemical system and the methods to compute Jacobians and integrate in time
            ...
            !> Target waters loop
                do i=1,this%num_target_waters_dom
                    if (abs(mixing_ratios%cols(i)%col_1(1)-1d0)<this%target_waters(this%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%CV_params%abs_tol .and. inf_norm_vec_real(mixing_ratios%cols(i)%col_1(2:mixing_ratios%cols(i)%dim))<this%target_waters(this%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%CV_params%abs_tol) then
                        ...
                        !if (this%target_waters(this%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species==this%target_waters(this%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species) then
                    ...
                    !> We solve reactive mixing iteration
                        call p_solver(target_waters_new(this%dom_tar_wat_indices(i)),p_prim(target_waters_old_old(this%dom_tar_wat_indices(i))),c_tilde,conc_nc,F_mat(i),Delta_t)
                    !> We compute equilibrium reaction rates from mass balance equation
                        call target_waters_new(this%dom_tar_wat_indices(i))%compute_r_eq(c_tilde(target_waters_new(this%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:target_waters_new(this%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species),Delta_t,F_mat(i))
                    !> Chapuza
                        ...
                        !> We compute concentrations of gases
                            call target_waters_new(this%dom_tar_wat_indices(i))%gas_chemistry%compute_conc_gases_iter(Delta_t,F_mat(i-this%num_ext_waters),target_waters_new(i)%volume,[target_waters_new(i)%r_eq,target_waters_new(i)%rk])
                    ...
                    !> We write output
                        if (k==this%chem_out_options%time_steps(kk) .and. this%dom_tar_wat_indices(i)==this%chem_out_options%ind_target_waters(ii)) then
                            !do j=1,this%target_waters(i)%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species
                                write(unit,"(I10,*(ES15.5))") this%dom_tar_wat_indices(i), (conc_nc(j), j=1,this%chem_out_options%num_aq_species)
                            !end do
    ...
    !            !> We compute concentrations of gases
    !                call target_waters_new(i)%gas_chemistry%compute_conc_gases_iter(Delta_t,F_mat(i-this%num_ext_waters),target_waters_new(i)%volume,[target_waters_new(i)%r_eq,target_waters_new(i)%rk])
    !            !> We compute volume of gas   
    !                call target_waters_new(i)%gas_chemistry%compute_vol_gas_conc()
    !            !> We compute activity coefficients of gases    
