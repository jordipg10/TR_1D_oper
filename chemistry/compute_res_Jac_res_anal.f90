...
subroutine compute_res_Jac_res_anal(this,indices_icon,n_icon,indices_constrains,ctot,dc2aq_dc1,log_Jacobian_act_coeffs,res,Jac_res)
    ...
    do i=1,n_icon(3)
        ...
        do j=1,this%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species
            Jac_res(indices_icon%cols(3)%col_1(i),j)=(10**(this%log_act_coeffs(indices_icon%cols(3)%col_1(i))))*(log_Jacobian_act_coeffs(indices_icon%cols(3)%col_1(i),j)+dot_product(log_Jacobian_act_coeffs(indices_icon%cols(3)%col_1(i),this%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:this%aq_phase%num_species),dc2aq_dc1(:,j)))*(this%concentrations(indices_icon%cols(3)%col_1(i))/this%concentrations(j))
        end do
